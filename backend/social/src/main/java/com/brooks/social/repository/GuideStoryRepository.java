...
public interface GuideStoryRepository extends JpaRepository<GuideStory, UUID> {
    ...
    List<GuideStory> findActiveStoriesByCreatorIds(List<UUID> creatorIds, Instant now, Pageable pageable);

    @Query("SELECT COUNT(s) FROM GuideStory s WHERE s.creatorId IN :creatorIds AND s.expiresAt > :now")
    long countActiveStoriesByCreatorIds(List<UUID> creatorIds, Instant now);
